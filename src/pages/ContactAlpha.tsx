...
function ContactAlpha({ preview, playerName }: AboutProps) {
  ...
  const [choices, setChoices] = useState<string[]>([
    "(choices1)画面二つ目になったよー",
    "(choices2)あ、chatGPTのリアルタイムで文字出るオプション",
    "(choices3)つけておいてーあれあったらだいぶ助かる",
    "(choice4)",
  ]);
      ...
      <Container maxWidth="xl">
        <Box sx={outerBoxStyle2}>
          <Box sx={playerBoxStyle}>
            <Box sx={{ ...playerNameBox, fontSize: 15, textAlign: "center" }}>
              {playerName}
            </Box>
            <Box
              sx={{
          ...
          <section>
            <div onMouseEnter={() => setChoicesNumber(0)}>
              <Box onClick={openDialog} sx={choicesBoxStyle2}>
                選択肢1
                <Grid container>
                  <Grid item xs={3}>
                    {choicesNumber == 0 && (
          ...
          <div onMouseEnter={() => setChoicesNumber(1)}>
            <Box sx={choicesBoxStyle} onClick={openDialog}>
              選択肢2
              <Grid container>
                <Grid item xs={3}>
                  {choicesNumber == 1 && (
