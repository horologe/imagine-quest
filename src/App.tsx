...
export const playCharacterImgContext = createContext<PlayCharacterImgContextType>({
  PlayerCharacterImg: '',
...
export const useCharacterIndexContext = createContext<useCharacterIndexContextType>({
  useCharacterIndex: 0,
...
function App() {
  ...
  const [PlayCharacterSaveText, setPlayCharacterSaveText] = useState<string>('');
  ...
  const rootElement = (
    <BrowserRouter>
      <useCharacterIndexContext.Provider value={{ useCharacterIndex, setUseCharacterIndex }}>
        <useCharacterContext.Provider value={{ useCharacter, setUseCharacter }}>
          <emailContext.Provider value={{ saveEmail, setSaveEmail }}>
            <choiceContext.Provider value={{ PlayerChoiceText, setPlayerChoiceText }}>
              <base64Context.Provider value={{ base64SaveText, setbase64SaveText }}>
                <PlayCharacterPromptContext.Provider value={{ PromptSaveText, setPromptSaveText }}>
                  <playCharacterImgContext.Provider value={{ PlayerCharacterImg, setPlayerCharacterImg }}>
                    <PlayCharacterContext.Provider value={{ PlayCharacterSaveText, setPlayCharacterSaveText }}>
                      <gameContext.Provider value={{ gameSaveText, setGameSaveText }}>
                        <PrologueContext.Provider value={{ PrologueSaveText, setPrologueSaveText }}>
                          <Routes>
                            ...
                            {isAuth && (
                              <>
                                ...
                                <Route path="CharacterChoose" element={<CharacterChoose playerName={playerName} setPlayerName={setPlayerName} />} />
                                <Route path="PlayerChoice" element={<PlayerChoice />} />
                                <Route path="CharacterMaking" element={<CharacterMaking playerName={playerName} handleChangeName={handleChangeName} />} />
                                <Route path="WorldMaking" element={<WorldMaking />} />
                                <Route path="/about" element={<About file={file} preview={preview} playerName={playerName} handleChangeFile={handleChangeFile} handleChangeName={handleChangeName} />} />
                                <Route path="/contact" element={<Contact preview={preview} playerName={playerName} />} />
                                <Route path="/ContactAlpha" element={<ContactAlpha preview={preview} playerName={playerName} />} />
                                <Route path="/GameProof" element={<GameProof file={file} preview={preview} gameFile={gameFile} playerName={playerName} />} />
